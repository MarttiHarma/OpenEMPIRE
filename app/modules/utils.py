...
def get_active_results(results_folders):
    """
    Recursively searches for any folder containing Output/ with .csv files.
    Returns the first parent folder found.
    """
    base_path = Path(results_folders[0])
    print("DEBUG: Base path to search:", base_path)

    if not base_path.exists():
        raise ValueError(f"Results folder {base_path} does not exist.")

    candidates = []

    for output_path in base_path.rglob("Output"):
        if output_path.is_dir():
            csv_files = list(output_path.glob("*.csv"))
            if csv_files:
                candidates.append(output_path.parent)

    if not candidates:
        raise ValueError("No valid results folders found under " + str(base_path))

    chosen = candidates[0]
    print("DEBUG: Selected results folder:", chosen)
...
def get_active_results(folders: list[Path]) -> Path:
    ...
    print("DEBUG: Called get_active_results()")
    print("DEBUG: results_folders =", valid_result_folders_dict)
    print("DEBUG: valid_result_folders_dict =", valid_result_folders_dict)
    print("DEBUG: results_folder_relative =", results_folder_relative)
